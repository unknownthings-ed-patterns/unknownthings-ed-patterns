...
### 読書レベルの三区分——「ちょうどよい」を知るための背景知識

Betts（1946）は読書レベルを次の三区分で整理した。IPICK は、独立レベルを自分で確認するための実践的ツールとして位置づけられる。

| レベル | 語認識率の目安 | 内容理解の目安 | 使い場面 |
|---|---|---|---|
| **フラストレーションレベル** | 90%未満 | 75%未満 | 一人読みには難しすぎる——IPICK のC・KでNOになる本 |
| **インストラクションレベル** | 95〜98% | 75〜90% | 教師のサポート付きで読める——Guided Reading・Read to Someone の対象 |
| **独立レベル** | 99%以上 | 90%以上 | 一人で快適に読める——独立読書・リーディング・ワークショップの自由読みに最適 |

**IPICKとの接続**：C（Comprehend）とK（Know the words）が両方YESであれば、独立レベルにあるサインと見なせる。Allington & McGill-Franzen（2013）は独立読書では「ちょっと難しい本」ではなく「確実に読める本（独立レベル）」が必要と強調する——語認識率99%以上・理解90%以上が目安。5本指ルールで「1〜2本以下」に相当するのが独立レベルだ。
